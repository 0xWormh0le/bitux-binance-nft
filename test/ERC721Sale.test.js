...
contract("ERC721Sale", (accounts) => {
    ...
    describe("buy", () => {
        it("works and costs something (minter:6)", async () => {
            ...
            assert.equal(buyerBalance.cmp(buyerBalanceBeforeSale.sub(new BN('' + messageValue))), -1);
            assert.equal(sellerBalanceBeforeSale.add(new BN('' + tokenPrice)).sub(sellerServiceFee).toString(), sellerBalance.toString());
        });
        ...
        it("fails when buying for another value (minter:6)", async () => {
            ...
            assert.equal(
                thrownError,
                "Error: Returned error: VM Exception while processing transaction: revert incorrect signature -- Reason given: incorrect signature."
            );
        });
        ...
        it("can't buy token again once it was bought.", async () => {
            const tokenId =
                "0x222222229bd51a8f1fd5a5f74e4a256513210caf2ade63cd25c7e4c654174666"; // Randomly chosen
            await mint(accounts[6], tokenId);
